...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    pa0 = PropSymbolExpr('PacmanAlive', time=0)
    pa1 = PropSymbolExpr('PacmanAlive', time=1)
    pb0 = PropSymbolExpr('PacmanBorn', time=0)
    pk0 = PropSymbolExpr('PacmanKilled', time=0)

    return conjoin([pa1%((pa0&(~pk0))|((~pa0)&pb0)),~(pa0&pb0),pb0])
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([ disjoin([ ~x for x in combi]) for combi in itertools.combinations(literals, 2)])    
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacman = PropSymbolExpr(pacman_str, x, y, time = now)
    return pacman % disjoin(possible_causes)
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorModel=None, successorAxioms=None))
    KB.append(PropSymbolExpr(action0, time=t))
    KB.append(pacphysicsAxioms(t+1, all_coords, non_outer_wall_coords, walls_grid, sensorModel=None, successorAxioms=allLegalSuccessorAxioms))
    KB.append(PropSymbolExpr(action1, time=t+1))
    model1 = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time=t+1)]))
    model2 = findModel(conjoin(KB + [~PropSymbolExpr(pacman_str, x1, y1, time=t+1)]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            test_wall.append(PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(test_wall))
        ...
        test_dir = []
        for dir in actions:
            test_dir.append(PropSymbolExpr(dir, time = t))
        KB.append(exactlyOne(test_dir))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in food:
            model1 = (PropSymbolExpr(pacman_str, x, y, time=t) & PropSymbolExpr(food_str, x, y, time=t))
            model2 = ~PropSymbolExpr(food_str, x, y, time=t+1)
            KB.append(model2 % model1 | ~PropSymbolExpr(food_str, x, y, time=t))   
